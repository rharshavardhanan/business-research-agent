...
def build(monkeypatch, tmp_path, password=None, username=None):
...
def test_no_password_configured_means_no_auth(monkeypatch, tmp_path):
    """The app is a local tool by default; a password is opt-in for deployment."""
    client = build(monkeypatch, tmp_path, password=None)
    assert client.get("/health").status_code == 200


def test_password_configured_blocks_anonymous_requests(monkeypatch, tmp_path):
    client = build(monkeypatch, tmp_path, password="s3cret")
    r = client.get("/health")
    assert r.status_code == 401
    assert "Basic" in r.headers.get("www-authenticate", "")


def test_correct_credentials_are_accepted(monkeypatch, tmp_path):
    client = build(monkeypatch, tmp_path, password="s3cret")
...
def test_username_is_configurable(monkeypatch, tmp_path):
    client = build(monkeypatch, tmp_path, password="s3cret", username="harshav")
...
def test_the_ui_itself_is_protected_not_just_the_api(monkeypatch, tmp_path):
...
def test_a_write_route_is_protected(monkeypatch, tmp_path):
    client = build(monkeypatch, tmp_path, password="s3cret")
    r = client.post("/workbooks", json={"path": "x.xlsx", "kind": "workbook"})
    assert r.status_code == 401


def test_malformed_authorization_header_is_rejected(monkeypatch, tmp_path):
    client = build(monkeypatch, tmp_path, password="s3cret")
